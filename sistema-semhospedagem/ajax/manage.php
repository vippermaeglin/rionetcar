...
elseif ( 'duplicar' == $action) {
	...
	*/
	...
	//comentei pois ainda quero  fazer testes 10/09/2014
	/*$sql  = "DELETE FROM team WHERE partner_id = ".$partner['id']; 
	$rs  = mysql_query($sql );		
	if(!$rs){		
		//Util::log( mysql_error() . " ". $sql);	
	}	*/
...
*/
...
else if ( 'teamremove_anunciante' == $action) {
	...
	if ( $order_count > 0 ) {
			Session::Set('notice', 'Esta oferta está contida em '.$order_count .' pedido(s) pago(s). Por motivo de segurança o(s) pedido(s) devem ser apagados primeiro.');
			json(null, 'refresh'); 
	}
	//ZTeam::DeleteTeam($id);
...
elseif('faqremove' == $action) {
...
}elseif('delimagem_revenda' == $action) {
	
	$imagem = "media/".$_REQUEST['imagem'];
	
	$sql  = "UPDATE user SET imagem = '' WHERE `id` = ".$id;
	$rs  = mysql_query($sql);
	$num = mysql_num_rows($rs);
	...
	if($rs) {
		Session::Set('notice', 'Removida com sucesso!');
		header("Location:".$ROOTPATH."/index.php?page=meusdados");
	} else {
		Session::Set('notice', 'Falha ao remover!');
		header("Location:".$ROOTPATH."/index.php?page=meusdados");
	}

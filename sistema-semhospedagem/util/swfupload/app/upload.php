...
// Check post_max_size (http://us3.php.net/manual/en/features.file-upload.php#73762)
	...
	$pathimages = "slideshowbannersheader";  

	$save_path = dirname(dirname(dirname(dirname(__FILE__)))) . "/media/".$pathimages."/";
...
// Validate file name (for our purposes we'll just remove invalid characters)
	...
	$idofertatemp = $_REQUEST['idofertatemp'];
	...
	Util::log("idoferta: ".$idoferta);

	Util::log("idofertatemp: ".$idofertatemp);
	...
	if($tipo=="team"){
		...
		$image = "team/{$year}/{$day}/{$n}"; 

		$path = IMG_ROOT . "/" . $image;

		

		move_uploaded_file($_FILES[$upload_name]["tmp_name"], $path); 
		...
		$sql = "insert into team_fotos (imagem,idoferta,idofertatemp,data) values ('$image','$idoferta','$idofertatemp',now()) ";
...
// Process the file
	...
	*/
	...
	if (!@move_uploaded_file($_FILES[$upload_name]["tmp_name"], $save_path.$file_name)) {
	 ...
	 $img->carrega($save_path.$file_name)->redimensiona( 75, 75)->grava($save_path."/thumbs/".$file_name);

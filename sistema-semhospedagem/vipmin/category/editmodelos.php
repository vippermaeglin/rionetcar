...
$id =  $_GET['id'] ;
$category = Table::Fetch('modelo', $id);
...
if (!$edicao) { // cadastro
	...
	if(!is_post()){
		include template('manage_category_editmodelos');
	}
	else{ 
	 
		$table = new Table('modelo', $_POST); 
		$table->id =  time() ;
		$uarray = array('id', 'fabricante',  'nome', 'tipo');
		 
		$flag = $table->insert($uarray);
		
		if($flag){
			Session::Set('notice', 'Dados cadastrados com sucesso.');
			redirect( WEB_ROOT . "/vipmin/category/indexmodelos.php");
		}
		else{

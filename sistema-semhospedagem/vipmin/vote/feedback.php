...
//View
	...
	if (!$feedback) {
		Session::Set('error', 'The survey feedback does not exist.');
		redirect( WEB_ROOT . '/vipmin/vote/feedback.php');
		exit;
	}
...
//deleted
	...
	if ($flag_feedback && $flag_feedback_question && $flag_feedback_input) {
		...
		Session::Set('error', 'Delete Failed');
	}

	redirect( WEB_ROOT . '/vipmin/vote/feedback.php?action=list');
	exit;
...
//Question Details
	...
	if (!$question) {
		Session::Set('error', 'This question does not exist.');
		redirect( WEB_ROOT . '/vipmin/vote/feedback.php');
		exit;
	}
...
//View custom input
	...
	if (!$options) {
		Session::Set('error', 'This option does not exist.');
		redirect( WEB_ROOT . '/vipmin/vote/feedback.php');
		exit;
	}

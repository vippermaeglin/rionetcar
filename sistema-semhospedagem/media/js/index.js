...
window.x_init_hook_click = function () {
	...
	jQuery('a.processar').click(function () {
        if (jQuery(this).attr('no') == 'yes') return false;
        var link = jQuery(this).attr('href');
        var ask = jQuery(this).attr('ask');
        if (link.indexOf('/delete') > 0 && !confirm('确定删除本条记录吗？')) {
        ...
        //processar();
		jQuery.colorbox({html:"<div class='msgsoft'><img src='../../media/js/ajax-loader.gif'>Estamos processando. Aguarde...</div>"});
		X.get(jQuery(this).attr('href'));
        return false
    });   
	
	jQuery('a.ajaxlink_edit').click(function () {
        if (jQuery(this).attr('no') == 'yes') return false;
        var link = jQuery(this).attr('href');
        var ask = jQuery(this).attr('ask');
        if (link.indexOf('/delete') > 0 && !confirm('确定删除本条记录吗？')) {
    ...
    jQuery('.remove-record').click(function () {
        return confirm('Você tem certeza que deseja apagar este registro?')
    });
    ...
    var e = function () {
        ...
        if (ls > 0) {
            ...
            var ls = parseInt(Math.round(ls % 60000) / 1000);
                var html = '<li><span>' + lh + '</span></li><li><span>' + lm + '</span></li><li><span>' + ls + '</span></li>'
            if (ld > 0) {
            } else {
                var html = '<li><span>' + lh + '</span></li><li><span>' + lm + '</span></li><li><span>' + ls + '</span></li>'
            }
...
X.misc.noticenext = function (tid, nid,qtdeenviohora,tipo) {
    jQuery('#dialog_subscribe_count_id').html(nid);
    return X.get(WEB_ROOT + '/ajax/manage.php?action=noticesubscribe&id=' + tid + '&nid=' + nid + '&qtdeenviohora=' + qtdeenviohora+ '&tipo=' + tipo)
};
...
X.misc.noticesms = function (tid, nid, qtdeenviohora) {
...
X.manage.loadtemplate = function (id) {
    window.location.href = WEB_ROOT + '/vipmin/system/template.php?id=' + id
};
X.manage.loadpage = function (id) {
    window.location.href = WEB_ROOT + '/vipmin/system/page.php?id=' + id
};

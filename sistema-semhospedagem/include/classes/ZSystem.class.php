...
class ZSystem
{
	...
	static private function BuildINI($ini) {
		...
		$ini['system']['wwwprefix'] = $protocol.$host . WEB_ROOT;
		$ini['system']['imgprefix'] = $protocol.$host . WEB_ROOT;
		if(!$ini['system']['sitename']) {
			$ini['system']['sitename'] = 'Vipcom';
		}
		if(!$ini['system']['sitetitle']) {
			$ini['system']['sitetitle'] = 'Compras Coletivas';
		}
		if(!$ini['system']['abbreviation']) {
			$ini['system']['abbreviation'] = 'Vipcom';
		}
		if(!$ini['system']['couponname']) {
			$ini['system']['couponname'] = 'Vipcom';
		}
		if(!$ini['system']['currency']) {
			$ini['system']['currency'] = '&#165;';
		}
		if(!$ini['system']['currencyname']) {
			$ini['system']['currencyname'] = 'USD';
		}
